...
def read_Lung_datasets(root_path, mode='train'):
    images = []
    masks = []

    image_root = os.path.join(root_path, 'train')
    #gt_root = os.path.join(root_path, 'resized-label')


    for image_name in os.listdir(image_root):
        if image_name[-10:] == '-label.png':
            image_path = os.path.join(image_root, image_name.split('-label')[0] + '.png')
            label_path = os.path.join(image_root, image_name)
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self,root_path, datasets='Messidor',  mode='train'):
        ...
        assert self.dataset in ['RIM-ONE', 'Messidor', 'ORIGA', 'DRIVE', 'Lung', 'Kidney', 'Liver','Colon','Colon_p', 'COVID-19', 'Cell', 'Vessel'], \
        ...
        if self.dataset == 'RIM-ONE':
            ...
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
        elif self.dataset == 'Kidney':
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
        elif self.dataset == 'Liver':
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
        elif self.dataset == 'Colon':
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
        elif self.dataset == 'Colon_p':
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
        elif self.dataset == 'COVID-19':
            self.images, self.labels = read_Lung_datasets(self.root, self.mode)
